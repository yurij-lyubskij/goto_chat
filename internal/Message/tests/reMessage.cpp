...
TEST(MessageTests, equatingVoiceMessage){
	...
	VoiceMessage mes3;
	obj.type = message;
	obj.attr = { "3" , "/statics/voice", "50", "1", "1", "1" };
	mes3 = (iMessage) obj;
	EXPECT_EQ(mes3.getId(), 3);
...
TEST(MessageTests, downCast){
	iMessage iMes1(1, "test", 50, 1, 1, textMessage);
	iMessage iMes2(1, "/test/statics/voice", 50, 1, 1, voiceMessage);
